...
**Action:** Throttle eviction sweeps with time.monotonic() so they run at most once per second, preserving O(1) performance on the hot path without breaking memory bounds.
