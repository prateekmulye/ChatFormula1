...
def render_settings_panel() -> None:
    ...
    with st.expander("⚙️ Settings", expanded=True):
        ...
        def confirm_clear_settings() -> None:
            st.write(
            ...
            with col_cancel:
                if st.button(
                    "Cancel", use_container_width=True, key="settings_clear_cancel"
                ):
                    st.rerun()
            with col_confirm:
                if st.button(
        ...
        def confirm_new_session_settings() -> None:
            st.write(
                "Are you sure you want to start a new session? The current conversation will be lost."
            )
            col_cancel, col_confirm = st.columns(2)
            with col_cancel:
                if st.button(
                    "Cancel", use_container_width=True, key="settings_new_cancel"
                ):
                    st.rerun()
            with col_confirm:
                if st.button(
